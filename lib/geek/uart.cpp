...
void geekUart::Send(vector<unsigned char> SendInfos)
{
    for(int i = 0;i < SendInfos.size();i++)
    {
        Base.send(&SendInfos[i],1);
...
void geekUart::Receive(unsigned char& ReceiveInfo)
{
...
void geekUart::Receive(short int& ReceiveInfo)
{
...
void geekUart::Receive(int& ReceiveInfo)
{
...
void geekUart::Receive(long int& ReceiveInfo)
{
...
void geekUart::Receive(long long int& ReceiveInfo)
{
    unsigned char ReceiveInfoGet[8];
...
// 串口初始化信息获取
void geekUart::initInfosGet(const char* DevicePath,_UartDeviceBaudrate_ UartDeviceBaudrate,_ComputerArchtecture_ ComputerArch)
{
...
// 串口接收数据清空
void geekUart::receiveInfosClear()
{
    ReceiveInfos.clear();
}
...
void geekUart::sendInfoAdd(unsigned char data)
{
    SendInfos.push_back(data);
}
void geekUart::sendInfoAdd(long long int data)
{
...
void geekUart::sendInfoAdd(long int data)
{
...
void geekUart::sendInfoAdd(int data)
{
    int* data_p = &data;
    unsigned char* data_add_p = (unsigned char*)data_p;
    for(int i = 0;i < 4;i++)
...
void geekUart::sendInfoAdd(short int data)
{
...
// 串口基础库
bool geekUart::Base::open(const char* portname, int baudrate, char parity, char databit, char stopbit, char localmode, char outputmode, char inputmode, char synchronizeflag)
{
    // 打开串口
    geekUart::Base::pHandle[0] = -1;
    // 以 读写、不阻塞 方式打开
    geekUart::Base::pHandle[0] = ::open(portname,O_RDWR|O_NOCTTY|O_NONBLOCK);
...
int geekUart::Base::send(const void *buf,int len)
{
    int sendCount = 0;
    if(pHandle[0] != -1)
    {   
...
int geekUart::Base::receive(void *buf,int maxlen)
{
