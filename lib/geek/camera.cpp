...
// 摄像头图像获取
void geekCamera::Capture()
{
    Mat ImgCapture;
    // 将视频流转为图像流
    if(CameraInitParameter.CameraMultiThreading == true)
    {
        thread CameraCapture_Thread (&geekCamera::captureThread,this,ref(Camera)); 
...
// 多线程摄像头图像获取
void geekCamera::captureThread(VideoCapture &Camera)
{
    Mat ImgCapture;
    while(true)
    {
